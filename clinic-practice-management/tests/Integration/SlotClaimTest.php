        ...
        $duplicate = $wpdb->query(
            $wpdb->prepare(
                'INSERT IGNORE INTO ' . $wpdb->prefix . 'cpms_schedule_slots
                 ...
                 VALUES (1, (SELECT id FROM ' . $wpdb->prefix . 'cpms_locations WHERE clinic_id = 1 AND is_primary = 1 LIMIT 1), %d, %s, %s, 20, 1, 1, %s, %s)', // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                $clinicianId,
                '2026-10-01',
                '09:00:00',
                $now,
