...
> **وضعیت جاری (2026-09-11):** مرجع معتبر = [`project-current-state.md`](project-current-state.md). خلاصه: **Phase 0 / 0.5 = CLOSED** · **Phase 1A = CLOSED · Phase 1B = DEFERRED** · **Phase 2 (Multi-Clinic Core) = IN PROGRESS** (C4/C5/C6 CLOSED؛ **C7 = CLOSED** — PR #20 MERGED، merge = `a385d868`؛ **پذیرش/بستن رسمی با تصمیم صریح مالک در 2026-09-11**؛ فقط دامنهٔ تعریف‌شدهٔ C7) · **C8 = CLOSED به‌عنوان بستهٔ شواهد/مستنداتِ فوندیشن Location (بدون پیاده‌سازی)** · **C9 = پیاده‌سازیِ محدودشده کامل / READY FOR ARCHITECT MERGE REVIEW روی PR #23 (DRAFT — DO NOT MERGE، base `0fd5c27`)؛ ‏CLOSED نیست** (فقط دو یافتهٔ A/Low که خودِ C7 معرفی کرده بود، در مرز REST؛ بدون تغییر Domain/Application، بدون migration، بدون ابزار گارْد جدید؛ قاعدهٔ لایه‌بندیِ بومی‌سازی و بدهیِ تحمّل‌شده در `project-current-state.md` §I‑1 و `phase2-state.md` §C9) · **C10 = NOT STARTED و مجاز نشده** · **Phase 3 = NOT STARTED** · آخرین migration = `0020` (‏`0021` ساخته/تصویب نشده).
