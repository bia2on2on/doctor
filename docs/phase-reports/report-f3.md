...
4. ~~**تأیید اجرای نهایی CI**~~ — **حل شد:** دسترسی GitHub بازیابی و CI با ۱۲ تکرار دیباگ واقعی (رفع باگ‌های §4.1 + زیرساخت §4.2) به **سبز کامل** رسید — runهای آخر: هر 5 job ✓ (Unit PHP 8.1–8.4 + Integration 99 تست/456 assertion). شواهد: run سبز کامل روی `fd191b0` (هر 5 job ✓، مشاهده مستقیم). دو کامیت بعدی (`380d74d` حذف دیاگ‌های موقت تست، `d999d37` همین مستندات) تغییری در کد Production ندارند؛ چون توکن GitHub این نشست مجدداً در همین نقطه منقضی شد، تأیید run روی HEAD نهایی پس از اتصال مجدد انجام می‌شود (همان سیاست «اجرای واقعی»).
