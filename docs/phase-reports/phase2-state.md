...
- **شواهد اجراشدهٔ موجود (واقعی):** اهداف مصوب `performance-baseline.md`؛
  ...
  Pilot/Staging `34651627290`) که **فقط اجرای موفق استپ/گیت را اثبات می‌کنند** — اعداد
  benchmark جاری از شواهد فعلی Arena قابل بازیابی نیستند و موفقیت workflow ≠ پاس‌شدن
  آستانه‌های latency/NFR. **این شواهد آستانه‌های NFR محیط مرجع/تجاری را پاس‌شده ثابت
  نمی‌کنند.**
