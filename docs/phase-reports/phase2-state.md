...
| | |
|---|---|
| **آخرین به‌روزرسانی** | **C9 (i18n) — پیاده‌سازیِ محدودشده کامل؛ READY FOR ARCHITECT MERGE REVIEW؛ ‏C9 بسته نشده** (2026-09-11). تحویل روی **PR #23 — DRAFT / DO NOT MERGE** با base = `0fd5c27`. پیش‌از این: پذیرش/بستن رسمی C7 با تصمیم صریح مالک + بسته‌شدن مستنداتی C8 (هر دو در sync فقط‌مستنداتِ PR #21 و PR #22). Checkpoint ادغام‌شدهٔ جاریِ main = `0fd5c27` (PR #22 MERGED 2026-09-11T15:56:52Z؛ والدین `b19930fe` + `c5f98ab9`). SHA history (تاریخی): `3fc5a54` → `becc82f` → `a49b182` (#14 head) → `099b644` (merge) → `248ca10` (PR #17 merge) → `4871f84` (PR #19 merge) → `6b438238` (#20 head) → `a385d868` (merge #20) → `3589b15d` (#21 head) → `b19930fe` (merge #21) → `c5f98ab9` (#22 head) → `0fd5c27` (merge #22) |
| **وضعیت Phase 2** | IN PROGRESS — C1..C6 done؛ **C6 CLOSED**؛ **C7 CLOSED — پیاده‌سازی ادغام‌شده در main (PR #20 MERGED، merge `a385d868`) و پذیرش/بستن رسمی با تصمیم صریح مالک در 2026-09-11 ثبت شد**؛ **C8 CLOSED به‌عنوان بستهٔ شواهد/مستنداتِ فوندیشن Location (بدون پیاده‌سازی — رجوع به §C8 پایین)**؛ **C9 = پیاده‌سازیِ محدودشده کامل و آمادهٔ بازبینیِ ادغام توسط معمار — ولی CLOSED نیست** (بستن رسمیِ C9 تصمیمِ تداومیِ پس‌از‌ادغام است؛ رجوع به §C9 پایین). **C10 NOT STARTED — مجاز نشده** |
| **آخرین remote SHA سبزِ تأییدشده** | `0fd5c27` (`origin/main`) — هر ۴ گیت پس‌از‌ادغام GREEN + مجموعاً ۱۹ check run همه success (پایین)؛ PR #22 **MERGED** (فقط مستندات) |
...
| **C9 (i18n — محدودشده)** | **پیاده‌سازیِ محدودشده کامل روی PR #23 (DRAFT — DO NOT MERGE، base `0fd5c27`)؛ READY FOR ARCHITECT MERGE REVIEW — CLOSED نیست.** فقط دو یافتهٔ A/Low که خودِ C7 معرفی کرده بود، در مرز REST translation-ready شدند؛ بدون تغییر Domain/Application، بدون migration، بدون ابزار گارْد جدید، بدون پاکسازی انبوه — §C9 پایین |
| **Schema** | `2026_09_09_0020` — بدون تغییر؛ فایل/تصویب `0021` وجود ندارد (بدون تغییر schema در C7؛ بستن مستنداتی C8 و پیاده‌سازیِ محدودشدهٔ C9 هم هیچ migrationای مجاز نمی‌کنند/نداشتند) |

> این فایل state جاری است، نه گزارش. عمداً به SHA کامیتِ خودِ این سند ارجاع
> نمی‌دهد — مبنا = checkpoint ادغام‌شدهٔ `0fd5c27` (`origin/main`؛ PR #22 MERGED — فقط مستندات).
...
## گیت‌های سبز — پس‌از‌ادغام (روی `0fd5c27` = `origin/main` جاری)
...
### C9 — پیاده‌سازیِ محدودشدهٔ i18n: کامل و آمادهٔ بازبینیِ ادغام (**CLOSED نیست**) — 2026-09-11

**وضعیت: «C9 bounded implementation complete / READY FOR ARCHITECT MERGE REVIEW».**
این **حداکثر** وضعیتِ مجازِ پیش‌از‌ادغام است. **بستن رسمیِ C9 یک تصمیمِ تداومیِ
پس‌از‌ادغام است** و فقط بر پایهٔ SHA واقعیِ ادغام‌شده در `main` + سبزیِ گیت‌های
پس‌از‌ادغامِ همان SHA انجام می‌شود. تا آن لحظه هیچ ادعای بستن ثبت نمی‌گردد و
در این سند هم ثبت نشده است.

**تحویل:** ‏**PR #23 — DRAFT / DO NOT MERGE**، base = `0fd5c27` (= `origin/main` جاری).
...
- ‏**GREEN** — پس از کامیتِ ترمیم، همان تست‌ها (بدون هیچ تغییری در خودشان) باید سبز
  شوند. نتیجهٔ نهایی در §«گیت‌ها» و در PR #23 ثبت می‌شود؛ **تا سبزیِ واقعیِ checks روی
  همان headِ PR ادعای GREEN ثبت نمی‌شود** و check معلق «سبز» نامیده نمی‌شود.
...
globalization اعلام نمی‌شود · هیچ کاتالوگِ ترجمه افزوده نشد · **C10 شروع نشده** و
عملکرد **اندازه‌گیری‌نشده** است · **Phase 2ِ Owner Roadmap بسته نشده** ·
‏**C9 بسته (CLOSED) نشده** · Phase 3 آغاز نشده · Migration `0021` نه ساخته و نه تصویب شد.

- **Do not start:** بستن رسمیِ C9 پیش از ادغام و پیش از سبزیِ گیت‌های پس‌از‌ادغام،
  ادغامِ PR #23 بدون بازبینیِ معمار، C10، Phase 3، Migration 0021، پاکسازیِ انبوهٔ
  بدهیِ i18n، ساختِ abstraction جدید فقط برای حذفِ دو literalِ تکراری.
...
> **C8 CLOSED به‌عنوان بستهٔ شواهد/مستندات (بدون پیاده‌سازی — §C8 بالا)** · **C9 = پیاده‌سازیِ
> محدودشده کامل و READY FOR ARCHITECT MERGE REVIEW روی PR #23 (DRAFT) — ولی CLOSED نیست**
> (بستن رسمیِ C9 تصمیمِ تداومیِ پس‌از‌ادغام است؛ §C9 بالا) · **C10 اجراه نشده و مجاز نشده**.
...
> برای C10 Performance: عملکرد **اندازه‌گیری‌نشده** است مگر شواهد benchmark واقعی وجود داشته باشد).
