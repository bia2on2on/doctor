...
| | |
|---|---|
| **آخرین به‌روزرسانی** | **آشتی نهایی مستندات فاز ۲ (2026-09-15 — فقط مستندات):** چک‌پوینت یکپارچهٔ زندهٔ `origin/main` = `35acced4993287bdffd1fb5c0ef2f8cb1634f99f` (ادغامِ PR #47؛ والدین `229b0b0` + `bb70079f`). **PHASE 2 — COMPLETE (تکمیل فنی؛ مسدودکننده‌های فنی بسته شدند)**: **M-2 بسته** (هر ۸ شغلِ کانونی روی main حل شده‌اند) · **M-4 بسته** (قراردادِ شکستِ قطعیِ نوع‌دارِ `NonRetryableJobFailure`) · **حقیقتِ زمانیِ Location** برای ساعاتِ سکوتِ `appt.reminder`/`fu.reminder` ادغام شد (اصلاحِ فیکسچرِ پس از M-4 نیز در #46 ادغام شد). **فاز ۳ — شروع نشده.** آخرین مهاجرت = `0020`؛ `0021` وجود ندارد و اختراع نشد. این ادعای انتشار/نسخهٔ تجاری **نیست**. جزئیات در بخش «آشتی نهایی فاز ۲» پایین. *قبلی:* **Slice 1B.1 (E) — اصلاحات پیش‌از‌ادغامِ یافته‌های بازبینیِ امنیتی/معماری روی Draft PR #27 (2026-09-12):** حذفِ `bindPayloadClinicScope` + scope-neutral شدنِ `ExportService` + گارْدِ محدودِ عضویت (`MembershipRepository::find_active`) با کدِ `CLINIC_EXPORT_CLINIC_NOT_AUTHORIZED` + fail-closed پیش‌فرضِ `JobsDispatcher`. جزئیات در بخش «Slice 1B.1» پایین. *قبلی:* **C10 (Performance review) — CLOSED؛ پذیرش/بستن رسمی با تصمیم صریح مالک در 2026-09-12** (بستهٔ شواهد یکپارچه در `bdb135e9`، PR #25 MERGED؛ فقط دامنهٔ محدودشدهٔ بازبینی شواهد — بدون ادعای NFR/بار/مقیاس‌پذیری؛ Phase 2 همچنان IN PROGRESS). *سابقه (2026-09-11):* بستهٔ شواهد عملکرد کامل / واجد شرایط فنی برای بستن؛ بستن رسمی مالک در انتظار. بستهٔ شواهدِ **فقط‌مستندات** در سند اختصاصی [`c10-performance-evidence.md`](c10-performance-evidence.md) + §C10 همین سند ثبت شد — بدون هیچ پیاده‌سازی/بهینه‌سازی/migration. پیش از این: **بستن رسمی C9 با تصمیم صریح مالک در 2026-09-11 پس از ادغامِ موفق** (PR #23 MERGED در `7146d5b`؛ بستن رسمی در sync فقط‌مستنداتِ PR #24 ثبت شد). Checkpoint ادغام‌شدهٔ جاریِ main = `bd2634a` (PR #24 MERGED 2026-09-11T21:54:50Z؛ والدین `7146d5b` + `2adec4e`). SHA history (تاریخی): `3fc5a54` → `becc82f` → `a49b182` (#14 head) → `099b644` (merge) → `248ca10` (PR #17 merge) → `4871f84` (PR #19 merge) → `6b438238` (#20 head) → `a385d868` (merge #20) → `3589b15d` (#21 head) → `b19930fe` (merge #21) → `c5f98ab9` (#22 head) → `0fd5c27` (merge #22) → `c92737b` (#23 head) → `7146d5b` (merge #23) → `2adec4e` (#24 head) → `bd2634a` (merge #24) |
| **وضعیت Phase 2** | **COMPLETE — تکمیل فنی (2026-09-15؛ مسدودکننده‌های فنی بسته شدند — رجوع به بخش «آشتی نهایی فاز ۲»)**. وضعیت پیشین تا پیش از ادغام‌های نهایی: IN PROGRESS — C1..C6 done؛ **C6 CLOSED**؛ **C7 CLOSED — پیاده‌سازی ادغام‌شده در main (PR #20 MERGED، merge `a385d868`) و پذیرش/بستن رسمی با تصمیم صریح مالک در 2026-09-11 ثبت شد**؛ **C8 CLOSED به‌عنوان بستهٔ شواهد/مستنداتِ فوندیشن Location (بدون پیاده‌سازی — رجوع به §C8 پایین)**؛ **C9 CLOSED — پیاده‌سازیِ محدودشده در main ادغام شد (PR #23 MERGED، merge `7146d5b`) و پذیرش/بستن رسمی با تصمیم صریح مالک در 2026-09-11 ثبت شد** (فقط دامنهٔ محدودشدهٔ تعریف‌شده؛ رجوع به §C9 پایین). **C10 CLOSED — پذیرش/بستن رسمی با تصمیم صریح مالک 2026-09-12؛ فقط دامنهٔ محدودشدهٔ بازبینی شواهد، بدون ادعای NFR/بار/مقیاس‌پذیری** (بستهٔ شواهدِ فقط‌مستندات؛ بدون هیچ پیاده‌سازی — §C10 پایین) |
| **آخرین remote SHA سبزِ تأییدشده** | `35acced4993287bdffd1fb5c0ef2f8cb1634f99f` (`origin/main` — راستی‌آزمایی زندهٔ 2026-09-15) — هر ۴ گیت پس‌از‌ادغام روی همین سر **success** (رویداد `push`): CI `34963801460` · Real WordPress Acceptance `34963801412` · Pilot/Staging Readiness Gate `34963801393` · Closure Gate `34963801453`؛ مجموعاً ۱۹ check-run، همگی `completed/success`. PR #47 **MERGED** 2026-09-15T11:32:03Z (تایم‌زونِ Location برای ساعاتِ سکوتِ یادآوری‌ها؛ والدینِ ادغام `229b0b0` + `bb70079f`). قبلی: `bdb135e9` (PR #25 — CI `34678813474` · Real‑WP `34678813477` · Pilot/Staging `34678813488` · Closure `34678813479`)؛ پیش از آن: `bd2634a` (PR #24 — CI `34651627032` · Real‑WP `34651627286` · Pilot/Staging `34651627290` · Closure `34651627211`) |
...
> نمی‌دهد — مبنا = checkpoint ادغام‌شدهٔ `35acced` (`origin/main`؛ ادغامِ **فنی**ِ
> نهاییِ فاز ۲: تایم‌زونِ Location برای ساعاتِ سکوتِ یادآوری‌ها؛ به‌روزرسانیِ پیشینِ مبنا
> `bd2634a` بود — PR #24 MERGED، فقط مستندات: بستن رسمی C9 + اصلاح واژگانی شواهد عملکرد).
...
## آشتی نهایی فاز ۲ — وضعیت پذیرفته‌شده (2026-09-15 — فقط مستندات؛ راستی‌آزمایی زنده)

**نتیجهٔ ثبت‌شده:**

| | |
|---|---|
| **وضعیت فاز ۲** | **PHASE 2 — COMPLETE** (تکمیل **فنی**؛ همهٔ مسدودکننده‌های فنی بسته شدند) |
| **وضعیت فاز ۳** | **Phase 3 — NOT STARTED** |
| **چک‌پوینت پایه (زنده)** | `origin/main` = `35acced4993287bdffd1fb5c0ef2f8cb1634f99f` — ادغامِ PR #47 در 2026-09-15T11:32:03Z؛ والدینِ ادغام: `229b0b0c6c03310bb6ee1415689f11b822703225` + `bb70079f3f46888c3123cc36d7054b82993f27bb` |
| **آخرین مهاجرت** | `2026_09_09_0020` — بدون تغییر؛ `0021` وجود ندارد و اختراع نشد |

**مرزهای این بیان (صریح):** «تکمیل فاز ۲» ادعای انتشار، نسخهٔ تجاری، آمادگی تجاری یا
تکمیل نقشهٔ راه **نیست**؛ معادلِ بسته‌شدنِ مسدودکننده‌های فنیِ ثبت‌شدهٔ فاز ۲ روی
`main` است. زمان‌بندی کامل/تعطیلات/فازهای بعدی در این بیان ادعا **نمی‌شوند**.

### وضعیت ادغام‌ها (راستی‌آزمایی زندهٔ 2026-09-15)

| مرجع | وضعیت |
|---|---|
| **PR #47** — تایم‌زونِ Location برای ساعاتِ سکوتِ یادآوری‌ها (سرِ تأییدشده `bb70079f`) | **MERGED** (ادغامِ فنیِ نهاییِ فاز ۲) |
| **PR #46** — تریاژ/اصلاحِ فیکسچرِ پس از ادغام در `FollowUpReminderLocationCalendarRedTest` (اصلاحِ فیکسچرِ پس از M-4) | **MERGED** |
| **PR #45** — سیاستِ پایانیِ شکست‌های قطعیِ نوع‌دار (قراردادِ `NonRetryableJobFailure`) = بستنِ **M-4** | **MERGED** |
| **PR #44** — پیش‌نویسِ اولیهٔ ترمینال‌سازی خطاهای قطعی | **بسته‌شده بدون ادغام** (مسیرِ جایگزینِ #45 ادغام شد) |
| **PR #43** — آخرین شغلِ فهرستِ کانونیِ M-2: `handwriting.gc` (بازتدوین به طبقهٔ **W**) | **MERGED** |
| **PR #42** — RED + اثباتِ وابستگیِ `backup.run` به تنظیماتِ سطح نصب؛ پذیرشِ کلیدهای پیکربندیِ بکاپ | **MERGED** |
| **PR #41** — کارگرِ نصب‌گستردهٔ `cleanup.oplog` (فقط تست — شواهد) | **MERGED** |
| **PR #40** — کلیدِ اسکالرِ سطح نصب `notif.archive_days` | **MERGED** |
| سایر ادغام‌های فاز ۲ (#26..#39 شامل برش‌های M-2، اسکوپِ شغل‌ها و اصلاحات پس از ادغام) | در تاریخچهٔ `main` ادغام شده‌اند |

### بستنِ فنی — آنچه بسته شد

1. **M-2 — بسته (۸ شغلِ کانونیِ فهرست، همه روی main حل شده):**
   `appt.reminder` · `visits.no_show` · `slots.generate` · `fu.reminder` ·
   `notif.dispatch` · `cleanup.oplog` · `backup.run` · `handwriting.gc`.
   ساختِ همگی **اسکوپ‌نوترال** است (وابستگی به `SettingsFactory`/`InstallationSettings`
   به‌جای `App::settings()`/`App::scope()` سراسری؛ کلاس‌ها در `JobScopeRegistry`:
   ۶ شغل `W` + `cleanup.oplog` و `backup.run` از نوع `S`). شغلی به نام `notif.purge`
   وجود ندارد و اختراع **نشده** است.
2. **M-4 — بسته با قراردادِ شکستِ قطعیِ نوع‌دار:**
   - نشانگرِ `NonRetryableJobFailure` (`src/Application/Jobs/NonRetryableJobFailure.php`):
     تصمیم **فقط بر پایهٔ نوعِ** Throwable، هرگز متنِ پیام.
   - موردِ صریحِ فعلی: **`JobPayloadInvalidException`** — payload نامعتبر = شکستِ قطعی
     و قابل‌بازتولید؛ در همان اولین تلاش `failed` نهایی می‌شود (`JobQueue::failTerminal`)،
     **بدون** بازصف/بک‌آفِ عمومی.
   - **قطعی ⇒ پایانی (`failed`) بدونِ بازصف/بک‌آفِ عمومی**؛ شکست‌های معمولی/گذرا همان
     مسیرِ قبلی را حفظ می‌کنند (`attempts < max_attempts` → بازصف با بک‌آف؛ وگرنه `failed`).
   - رفتارِ **قفلِ مانده/بازیابی (stale-lock) جدا و بدون تغییر** است.
   - ادعا **نمی‌شود** که همهٔ مواردِ `ScopeRequiredException` یا همهٔ خطاهای پیکربندی
     پایانی‌اند — نشانگر عمداً به خانواده‌های عمومی تعمیم داده **نشده** است.
3. **حقیقتِ زمانیِ Location برای مسیرهای قابل‌دسترسِ یادآوری (ادغامِ #47):**
   ساعاتِ سکوتِ ارسالِ `appt.reminder` و `fu.reminder` با **تایم‌زونِ صریح و ماندگارِ
   Location** ردیفِ عملیاتی (`cpms_locations.timezone`) و همان **لحظهٔ مرجعِ کنترل‌شدهٔ
   UTC** که پردازشِ یادآوریِ همان مسیر استفاده می‌کند ارزیابی می‌شود. تایم‌زونِ کلینیک
   در این مسیرها جایگزینِ Location نمی‌شود؛ نبود/خرابیِ تایم‌زونِ Location **پیش از
   ارزیابیِ ساعاتِ سکوت** به‌صورتِ شکستِ بسته رد می‌شود. فقط منبعِ منطقه‌زمانی/لحظهٔ
   مرجع تغییر کرد — آغاز/پایانِ پیکربندی‌شده، بازهٔ شب‌گذر، فعال‌سازی و ایجادِ اعلانِ
   داخلی بدون تغییر ماندند.
4. **تنظیماتِ سطح نصب (فقط مالکیتِ پذیرفته‌شده):**
   - `notif.archive_days` → `InstallationSettings::getNotifArchiveDays()`
     (Option وردپرس `cpms_notif_archive_days`؛ پیش‌فرض مؤثر `90`).
   - `retention.oplog_days` → `InstallationSettings::getOplogRetentionDays()`
     (Option وردپرس `cpms_retention_oplog_days`؛ پیش‌فرض مؤثر `90`).
   - کلیدهای پذیرفته‌شدهٔ بکاپ (مسیرِ #42): `backup.enabled`، `backup.interval_hours`،
     `backup.keep_count`، `backup.storage_path` — پیکربندیِ سطح نصب؛
     `backup.last_run_at` = **حالتِ عملیاتیِ سطح نصب**.
   - هیچ کلیدِ دیگری صرفاً به‌خاطرِ پیشوند مهاجرت/طبقه‌بندی **نشد**.

### گیت‌ها روی سرِ دقیقِ جاریِ `main` (راستی‌آزمایی زنده)

روی `35acced4993287bdffd1fb5c0ef2f8cb1634f99f` — هر ۴ گیتِ موردِ نیاز، رویداد `push`،
همگی `completed/success`؛ مجموعاً ۱۹ check-run، هیچ‌کدام معلق/ناموفق نیست:

| گیت | Run |
|---|---|
| CI | `34963801460` |
| Real WordPress Acceptance | `34963801412` |
| Pilot/Staging Readiness Gate | `34963801393` |
| Closure Gate | `34963801453` |

### پس‌زمینهٔ غیرمسدودکنندهٔ ثبت‌شده (تبدیل به «پاس» **نشده** است)

اقلامِ زیر همچنان **باز/اندازه‌گیری‌نشده** باقی می‌مانند و صرفاً از «مسدودکنندهٔ فاز ۲»
به «پس‌زمینهٔ شناخته‌شده» منتقل شدند — هر ادعای تکمیلِ آن‌ها نیازمندِ شواهدِ جداگانه است:

- **انتسابِ تننت/طراحیِ `cpms_operational_logs`** — پیاده‌سازی نشده.
- **مالکیتِ معناییِ حل‌نشدهٔ کلیدهای نصب/کنترل‌پلینِ نامرتبط** (مثل `license.server_url`،
  `queue.no_show_grace_minutes`، `update.*`) — آشتیِ جداگانه لازم دارد.
- **کران‌مندیِ پاک‌سازیِ غیر از ۸ شغلِ فهرستِ کانونی — در هر مورد که هنوز اندازه‌گیری
  نشده باشد** — اندازه‌گیری نشده.
- **نگهداری/کاراییِ جدولِ شغل‌ها — در هر مورد که هنوز اندازه‌گیری نشده باشد** —
  اندازه‌گیری نشده.
- اقلامِ ثبت‌شدهٔ پیشین (S2 ستونِ تننتِ صف — که به‌صورتِ مستند در سطحِ قراردادِ رجیستری
  حل شد نه ستونِ داده، و سایر اقلامِ مؤکول) — بدون تغییر در ماهیت.

### مرزِ نقشهٔ راه (وارد فاز ۲ **نشده‌اند**)

مجوزدهیِ کاملِ نقشه‌ها/نقش‌ها · `ServiceOffering` · قیمت‌گذاری · تعطیلات/زمان‌بندیِ کامل ·
فهرست انتظار · رزروِ عمومی · پرداخت · چت/ویدیوی امن · مجوز/قابلیت‌های تجاری — همه در
جایگاهِ خودِ نقشهٔ راه مالک باقی‌اند و این آشتی آن‌ها را به فاز ۲ **نمی‌آورد**.
...
> *نشانِ تاریخی (2026-09-15):* بندِ وضعیتِ بالا مربوط به **مقطعِ برشِ 1B** است و به‌عنوانِ
> سابقهٔ میانی حفظ شده است؛ وضعیتِ نهاییِ فاز ۲ (**کامل — فنی**) و بسته‌شدنِ M-2/M-4 و
> ادغامِ تایم‌زونِ Location در بخشِ «آشتی نهایی فاز ۲» ثبت شده است.
...
> **به‌روزرسانی آشتی (2026-09-15):** فهرستِ «پیاده‌سازی نشده» بالا وضعیتِ **همان مقطعِ
> برشِ 1B** را توصیف می‌کند و به‌عنوانِ سابقهٔ میانی حفظ شده است. پس از آن مقطع: (۱) هشت
> شغلِ کانونیِ فهرستِ M-2 روی main حل شدند (بستنِ M-2)؛ (۲) خطاهای قطعیِ پیلود با قراردادِ
> `NonRetryableJobFailure` پایانی شدند (بستنِ M-4 از طریقِ PR #45)؛ (۳) تایم‌زونِ
> صریحِ Location برای ساعاتِ سکوتِ یادآوری‌ها ادغام شد (PR #47)؛ و در نتیجه **وضعیتِ
> فاز ۲ = کامل (فنی)** ثبت شد — جزئیات در بخشِ «آشتی نهایی فاز ۲».
...
### M-2 — هر ۸ شغلِ کانونی روی main حل شده‌اند — **بسته** (ادغامِ نهایی: #43 → پس از آن #47 = چک‌پوینت `35acced`)

> **وضعیتِ نهایی (آشتی 2026-09-15 با چک‌پوینت زندهٔ `35acced`):** **هر ۸** شغلِ فهرستِ
> کانونیِ این بخش — `appt.reminder`, `visits.no_show`, `slots.generate`, `fu.reminder`,
> `notif.dispatch`, `cleanup.oplog`, `backup.run`, `handwriting.gc` — **روی main حل
> شده‌اند** (ساختِ اسکوپ‌نوترال؛ با کدِ جاریِ دیسپچر/سرویس‌ها تأیید شده).
> شغلِ `handwriting.gc` از طریق **ادغامِ #43** واردِ main شد. *عکسِ فوریِ پیشینِ همین بند
> («هفت شغل روی main + #43 باز») توصیفِ وضعیتِ میانیِ همان روز بود و اکنون با این وضعیتِ
> نهایی جایگزین شده است.*

> ✅ **`handwriting.gc` — آخرین آیتمِ کانونیِ فهرستِ M-2؛ ادغام‌شده روی main از طریقِ PR #43 (سر `da71e6d31d42b849a403006e0cd826c23ab01c9d`):**
...
### M-4 — **بسته** با قراردادِ شکستِ قطعیِ نوع‌دار (ادغامِ #45؛ پیش‌نویسِ جایگزین‌شدهٔ #44 بدون ادغام بسته شد)

**وضعیتِ نهایی (آشتی 2026-09-15):**

- قراردادِ صریحِ `NonRetryableJobFailure` (نشانگر — تصمیم فقط بر پایهٔ **نوعِ**
  Throwable): هر خطای دارای نشانگر در **همان اولین تلاش** نهایی می‌شود
  (`JobsDispatcher` → `JobQueue::failTerminal()` ⇒ `failed` نهایی، **بدونِ**
  بازصف/بک‌آفِ عمومی).
- موردِ صریحِ فعلی = **`JobPayloadInvalidException`** (payload نامعتبر/قطعی؛
  اجرای مجدد همان ورودی همان شکست را بازتولید می‌کند).
- شکست‌های معمولی/گذرا **همان مسیرِ عمومیِ قبلی را حفظ می‌کنند**
  (`attempts < max_attempts` → بازصف با بک‌آف؛ وگرنه `failed`).
- رفتارِ **قفلِ مانده/بازیابی (stale-lock) جدا و بدون تغییر** است.
- تعمیمِ نشانگر به خانواده‌های عمومی (همهٔ مواردِ `ScopeRequiredException` یا همهٔ
  خطاهای پیکربندی) انجام **نشده** و ادعا نمی‌شود — هر مورد تصمیمِ محصولیِ جداگانه
  می‌خواهد.

*سابقهٔ میانی (تاریخی — مربوط به برشِ 1B، پیش از ادغامِ #45):* «شکست‌های قطعیِ
اسکوپ/پیکربندی همچنان از رفتارِ بازصفِ عمومیِ `JobQueue::fail()` استفاده می‌کردند و
این آیتم به‌صورتِ follow-up باز بود.» آن وضعیت با ادغامِ **PR #45** بسته شد.
...
> `34709238433` همان job سبز شد. **ریشهٔ دقیقِ شکستِ `34706037675` = UNKNOWN**
> (لاگِ خامِ همان job از این sandbox قابل بازیابی نیست)؛ این سبزی‌ها شواهدی
> علیهِ رگرسیونِ قطعیِ محصول هستند، **نه** اثباتِ کلاس C. جزئیات در بخشِ
> «طبقه‌بندیِ `34706037675`» پایین.
...
#### طبقه‌بندیِ `34706037675` — **UNKNOWN** (ریشهٔ دقیق از شواهدِ موجود قابلِ اثبات نیست)

- **واقعیتِ run:** `34706037675` (pull_request · Real WordPress Acceptance ·
  head `b9d4e36`) در job با prefix `clinic_` **شکست خورد**؛ گامِ شکست‌خورده =
  «Browser acceptance — all 5 roles» (Playwright). گام‌های نصب/migration/
  راستی‌آزماییِ DB و «Log gate — بدون Fatal/Critical» در همان job success بودند.
- **ریشهٔ دقیقِ خروجِ غیرصفر: UNKNOWN.** لاگِ خامِ همان job از این محیط قابلِ
  بازیابی نیست (میزبانِ لاگِ Actions مسدود است)؛ پس نمی‌توان اثبات کرد که خروجیِ
  غیرصفر از شکستِ assertionِ محصول بود یا از خطای مرورگر/harness یا از نوسانِ
  محیطی. طبقهٔ C/D/A/B **بدونِ شواهدِ کافی اعمال نمی‌شود**.
- **مالکیتِ شواهدِ «307 passed / 0 failed»:** خلاصهٔ منتشرشده با عنوان
  `(job: success)` و متنِ `== rwp-acceptance: 307 passed / 0 failed ==` متعلق به
  run **push موفقِ همان SHA** (`34706035279`) است (jobِ `clinic_` آن در 16:47:34Z
  success شد و کامنت در 16:47:31Z منتشر شد) — **نه** به run شکست‌خوردهٔ
  `34706037675` (که jobِ `clinic_` آن در 16:48:18Z شکست خورد و هیچ خلاصه‌ای منتشر
  نکرد). بنابراین این خلاصه **نباید** به `34706037675` نسبت داده شود.
- **طبقه‌بندی نهایی: UNKNOWN.** سبز بودنِ همان SHA در run موازیِ push
  (`34706035279`) و سبز بودنِ headهای نهاییِ بعدی، شواهدی **علیهِ** «رگرسیونِ
  قطعیِ محصول» هستند، ولی این‌ها طبقهٔ C را **اثبات نمی‌کنند** — موفقیتِ بعدی،
  چیزی را که run شکست‌خورده واقعاً نشان داد بازنویسی نمی‌کند.
...
>
> *به‌روزرسانی آشتی نهایی (2026-09-15):* بندهای (B) طبقه‌بندیِ T/S/W و (C) تایم‌زونِ عملیاتیِ
> Location در مسیرهای قابل‌دسترسِ یادآوری از این فهرستِ «پیاده‌سازی‌نشده» **خارج شدند** —
> طبقه‌بندی اکنون در `JobScopeRegistry` اجرا می‌شود و ساعاتِ سکوتِ یادآوری‌ها با تایم‌زونِ
> صریحِ Location ارزیابی می‌شود (ادغامِ #47). بندهای باقی‌مانده (انتسابِ تننت در لاگِ عملیاتی،
> قواعدِ بک‌فیل، و کلیدهای کنترلیِ حل‌نشده) همچنان به‌عنوانِ پس‌زمینهٔ غیرمسدودکننده ثبت‌اند.
...
**به‌روزرسانی آشتی نهایی (2026-09-15):** برای دو مسیرِ قابل‌دسترسِ یادآوری
(`appt.reminder` و `fu.reminder`)، مصرفِ قاعدهٔ بالا در فاز ۲ انجام شد: ساعاتِ سکوتِ
ارسالِ اس‌ام‌اس در این دو مسیر اکنون با **تایم‌زونِ صریح و ماندگارِ Location** ردیفِ
عملیاتی (`cpms_locations.timezone`) و همان لحظهٔ مرجعِ کنترل‌شدهٔ پردازشِ یادآوری
ارزیابی می‌شود؛ تایم‌زونِ کلینیک جایگزینِ Location نمی‌شود و نبود/خرابیِ تایم‌زونِ
Location پیش از ارزیابی به‌صورتِ شکستِ بسته رد می‌شود (ادغامِ PR #47 در چک‌پوینت
`35acced`). این تکمیلِ فنیِ فاز ۲ است و **ادعای تکمیلِ زمان‌بندی/تعطیلات یا فاز ۶ را
ندارد**؛ سایر مصرف‌کنندگانِ رانتایم که هنوز از تایم‌زونِ کلینیک استفاده می‌کنند در وضعیتِ
پیشین باقی‌اند و در همین بند به‌عنوانِ قلمِ آشتیِ بعدی ثبت شده‌اند.
...
> با تصمیم صریح مالک 2026-09-11 پس از ادغام** — §C9 بالا) · **C10 = CLOSED (پذیرش رسمی مالک 2026-09-12؛ فقط دامنهٔ محدودشدهٔ شواهد — §C10 بالا)**. صف داخلی C1..C10 تکمیل شد؛ **در آن مقطع: Phase 2 همچنان IN PROGRESS** — End Gate تعریف/اجرا نشده.
> **به‌روزرسانی 2026-09-15 (آشتی نهایی):** همهٔ مسدودکننده‌های فنیِ ثبت‌شدهٔ فاز ۲ بسته
> شدند (بستنِ M-2 برای ۸ شغل کانونی؛ بستنِ M-4 با قراردادِ خطای قطعیِ نوع‌دار؛ ادغامِ
> تایم‌زونِ صریحِ Location برای ساعاتِ سکوتِ یادآوری‌ها در PR #47؛ چک‌پوینت `35acced`) ⇒
> **وضعیتِ فاز ۲ = کامل (فنی)**؛ فاز ۳ همچنان شروع نشده. برچسبِ تاریخیِ «End Gate
> (۲۶بندی)» هیچ‌گاه یک فهرستِ پذیرشِ تعریف‌شده نبود و مبنای این بستن، مسدودکننده‌های
> فنیِ ثبت‌شده است — نه بازسازیِ آن برچسب؛ ادعای انتشار/نسخه ۱ هم ساخته نمی‌شود.
