...
### [2026-09-10 ~15:00 UTC] — ایجنت Arena (شاخهٔ `arena/01a08b72-doctor`، PR #18 DRAFT) — C6 post-closure corrective: هفت مسیر مالی قفل‌شده روی Clinic ID 1 + نقطهٔ کور Tripwire — STOP
- **ماموریت کارفرما:** بازیابی حقیقت remote، راستی‌آزمایی مستقل چهار گیت post-merge، و **فقط در صورت سبزی آن‌ها** اجرای یک corrective باریک روی C6 (نه C6 redo، نه C7، نه Phase 3، نه سیاست Location authorization، بدون migration).
- **§۱ حقیقت remote (همه VERIFIED از GitHub API/git، نه از حافظهٔ chat):** `origin/main` = `6c84316` و **جلوتر نرفته**؛ PR #15 = MERGED با merge commit `6c84316` و والدین دقیقاً `099b644` + `4b3896c`؛ PR #13 = OPEN + DRAFT (**دست‌نخورده**); آخرین migration = `0020_idempotency_clinic_scope` (بدون 0021).
- **§۲ چهار گیت post-merge روی `6c84316` (همه VERIFIED success، run_attempt=1، head_branch=main):** CI `34469323541` · Real-WP `34469323447` · Pilot `34469323466` · Closure `34469323482`. تنها پس از این راستی‌آزمایی پیاده‌سازی شروع شد.
- **§۵ بازتأیید هفت سایت روی main:** هر هفت **موجود** بودند و هیچ‌کدام قبلاً اصلاح نشده بود: `ServiceRepository::all()` (bind `[1]`)، `PaymentRepository::revenueSummary()/forRange()/nextPaymentNumber()` (bind `[1, …]`)، `InvoiceRepository::openInvoices()/nextInvoiceNumber()` (bind `[1, …]`)، `FinanceService::lockClinic()` (bind `[1]`).
- **§۷ insert-failure — تأیید شد (Class B):** `CpmsDb::$strict` در runtime خاموش است (`setStrict` فقط در `MigrationRunner`)، پس `ensureNoSqlError()` no-op است؛ `wpdb::insert()` در خطا `false` می‌دهد ولی `wpdb::$insert_id` مقدار insert موفقِ قبلی را نگه می‌دارد. `InvoiceRepository::insert()` آن bool را **کامل دور می‌ریخت** و `issueInvoice` بدون هیچ گاردی `insertItem($invoiceId, …)` را صدا می‌زد ⇒ چسبیدن اقلام به فاکتور بیگانه + commit. گارد قدیمی مسیر پرداخت (`!$ok || $paymentId <= 0`) عملاً فقط «صفر» را می‌گرفت چون `$ok` همان insert_id بود. **اصلاح:** استثنا در `insert()/insertItem()/insertAdjustment()` هر دو repository + گارد دوم در `FinanceService` پیش از چسباندن اقلام؛ ROLLBACK؛ تشخیص Idempotency-race حفظ شد (فقط روی شکست واقعی درج). **بدون بازطراحی transaction** — scope باریک ماند.
- **§۸ Tripwire — نقطهٔ کور ابتدا اثبات شد، سپس بسته شد:** اسکن خط‌محور روی main گزارش می‌داد `CLEAN: 173 فایل، 0 نقض` درحالی‌که هر هفت سایت bind literal 1 داشتند. پاس جدید **statement-oriented و tenant-aware**: strip کامنت با حفظ offset → split statement → inline کردن fragmentهای SQL داخل متغیر (`$where = 'clinic_id = %d' . …`) → بازسازی SQL از literalها → محاسبهٔ ایندکس placeholder tenant → flag **فقط** وقتی bind همان literal `1`/`'1'` باشد. پوشش: `[1, …]`، prepared چندخطی، و الگوی row-lock/select روی `cpms_clinics`.
- **§۸ shadowing الگوی `LIMIT 1` — تأیید و رفع:** `limit_1_generic` در `NEGATIVE_EXCLUDES` بود و excludes **پیش از** positives و **خط‌به‌خط** بررسی می‌شدند؛ چون خودِ `select_first_clinic` همیشه `LIMIT 1` دارد، آن الگو **مرده** بود (با probe اجرایی روی خود ماژول اثبات شد: `positives=['select_first_clinic']` ولی `benign=['limit_1_generic']`). حذف شد و `select_first_clinic` با `(?:(?!%d).)*` باریک شد تا lookup صریح `WHERE id = %d` را نگیرد. **هیچ allowlistی اضافه نشد** (خالی ماند) و هیچ تشخیص موجودی تضعیف نشد.
- **شواهد اجرایی Tripwire (محلی، قابل تکرار):** روی mainِ پیش‌از‌corrective (`6c84316`) = **7 hardcode، exit 1** با file:line دقیق هر هفت؛ روی درخت اصلاح‌شده = **0 hardcode، exit 0** (۱ suspect واقعی و guard شده در `SystemClinicResolver` که با `COUNT(*) !== 1` fail-closed است)؛ self-test = **55 passed / 0 failed** (پیش از این 34؛ +21 حالت bound-parameter شامل «badهای تاریخی reject»، «integer 1 مشروع accept»، «متغیر صریح tenant accept»، «چندخطی/prepared»).
- **§۹ تست‌ها:** `tests/Integration/FinanceClinicScopeTest.php` — ۱۲ تست روی **MySQL واقعی** از مسیر واقعی `FinanceService`/repositoryها (بدون mock): Clinic فعال با id ≠ 1، ایزولهٔ مالی دو کلینیک، عدم دریافت فاکتور باز/پرداخت/تعرفهٔ Clinic دیگر، **عدم نشت نام و MRN**، شماره‌گذاری مستقل INV و PAY، هدف‌گرفتنِ قفل روی همان Clinic (با مشاهدهٔ SQL واقعی از فیلتر `query` وردپرس)، توقف امن روی شکست درج، سازگاری Clinic 1، قرارداد صریح Clinic (Reflection). پوشش ایزولهٔ موجود C6 دست‌نخورده ماند و هیچ مقدار انتظاری برای «مبارک‌کردن» رفتار خراب تغییر نکرد.
- **تزریق خطا (بدون mock، با نقض واقعی UNIQUE):** برای شکستِ درج، شماره‌ای «مسموم» درج می‌شود که **الگوی LIKE عددگیری را match می‌کند ولی regex آن را نه** (`INV-<ymd>-zzz`) ⇒ `MAX()` مقدار غیرقابل‌تجزیه می‌دهد ⇒ عددگیر `-001` می‌سازد ⇒ همان شماره از پیش اشغال است ⇒ نقض `u_inv_number`/`u_pay_number`. **نکتهٔ مهم:** poison بیرون از پیشوند LIKE (مثل `ZZZ-POISON`) هرگز وارد `MAX()` نمی‌شود و هیچ خطایی تزریق نمی‌کند.
- **طبقه‌بندی خطاها (همه صادقانه):**
  - **PR #16 (نشست قبلی، شاخهٔ `arena/01a08b0a-doctor`) قرمز بود** — CI `34480690937` / Integration: `Tests: 628, Assertions: 4077, Failures: 2` و هر دو همان تست‌های insert-failure. ریشه با شبیه‌سازی `MAX()`+regex اثبات شد: poison بیرون از LIKE ⇒ **Class D (نقص test-harness)**، نه رگرسیون محصول.
  - **خطای میانی خودم روی `2c8db27`** — CI `34488867070` / Integration: `Tests: 628, Assertions: 3940, Failures: 1` فقط `testClinicBCannotReceiveClinicAInvoiceOrPaymentRows`. ریشه: fixture مقدار MRN را با پسوند یکتاساز ذخیره می‌کند ولی تست `assertSame` روی مقدار خام داشت ⇒ **Class D**؛ fix در `4d0b83a`. (۱۱ تست دیگر همان دور پاس شدند، از جمله **هر دو تست insert-failure** — تأیید مستقلِ درستی استراتژی تزریق خطا.)
  - هفت نقص مالی و insert-failure = **Class B** (از پیش موجود). **هیچ Class A پیدا نشد.**
- **گیت‌های نهایی روی `4d0b83a` — همه VERIFIED success:** CI(PR) `34489635118` (Tripwire + Unit 8.1/8.2/8.3/8.4 + Integration WP6.7/MySQL8 + PHPStan lvl3 + WPCS changed-code) · Real-WP(PR) `34489635097` · Real-WP(push) `34489627769` · Closure(push) `34489627580` · Pilot/Staging(push) `34489627762`.
- **محدودیت محیط (Honesty):** sandbox فاقد PHP/MySQL است و Debian/packagist در allowlist شبکه نیستند ⇒ هیچ تست PHP **محلی** اجرا نشد؛ تنها اجراکننده GitHub Actions بود. لاگ خام jobها هم در این محیط EOF می‌دهد (blob storage مسدود)، بنابراین **شمار دقیق تست‌ها در اجرای سبز NOT MEASURED** است و از annotation اجرای قبلی (`628 tests`) **استنتاج** می‌شود.
- **Migration:** هیچ. **`0021` ساخته نشد.** **C7 = NOT STARTED.** بدون tag/release/version-bump.
- **STOP طبق دستور:** PR #18 = **DRAFT و OPEN، merge نشد**؛ PR #13 دست‌نخورده؛ `main` مستقیم تغییر نکرد؛ بدون force-push و بدون بازنویسی تاریخچه.
