...
### [2026-09-15 ~18:37 UTC] — ایجنت Arena (شاخهٔ `arena/01a0a629-doctor`) — رفع fail-open خواندن کارکنان PR #50 — STOP
- **فاز/محدوده:** فقط blocker تأییدشدهٔ خواندن در `StaffManagementPage` و اصلاح Acceptance متناظر؛ بدون Phase 3، بدون تغییر permission model، Settings/SMS/REST/Job، schema یا migration. وضعیت آغاز: PR #50 = OPEN+DRAFT+CLEAN روی `7022aa2` با شاخهٔ head متفاوت (`arena/01a0a5ec-doctor`)؛ به‌علت قفل نشست، همهٔ pushها فقط به `arena/01a0a629-doctor` انجام شد و PR جدیدی ساخته نشد.
- **RED معتبر:** run `35004230523` روی `12acbe9` با bootstrap/fixture/render واقعی: **13 تست، 176 assertion، 4 failure**؛ نشت ردیف‌های A/B برای Administrator بدون Membership، درخواست A→B، context چندکلینیکی مبهم و context غیرمثبت را اثبات کرد. بنابراین حالت `7022aa2` صرفاً چون writeها deny می‌شدند و Acceptance قبلی سبز بود، امن نبود.
- **رفع محصول:** `listUsers(int $clinicId)` برای `clinic_id <= 0` فوراً `[]` برمی‌گرداند؛ هر query کاربر به Membership همان Clinic محدود است؛ `clinicianLinkMap()` نیز fail-closed و با predicate آمادهٔ `clinic_id = %d` Clinic-scoped شد؛ parsing انتخاب Clinic فقط integer معتبر را می‌پذیرد. nonce/CSRF و مسیر write دست‌نخورده ماند.
- **تست/Acceptance:** fixtureها Organization و Clinic پویا می‌سازند و هیچ first-row/Clinic-1 fallback ندارند. پوشش render واقعی شامل admin با global cap و بدون Membership (هیچ ردیفی)، مدیر A با درخواست B (هیچ B)، مدیر با context معتبر A (A دیده می‌شود و B نه)، context چندکلینیکی مبهم، non-positive و clinician/person cross-Clinic است. Acceptance با مدیر Clinic seedشده برای رفتار مجاز desktop/tablet/mobile اصلاح شد و denial Administrator بدون Membership حفظ شد.
- **شواهد:** focused GREEN `35004847682` = **14/195**؛ Integration `35005444036` روی `0aed6d5` و اجرای exact-product-head `35007660036` روی `f750719` = هر دو **790/8767**؛ Unit و PHPStan در `35007660036` سبز؛ WPCS correction-delta در همان run = **PASS، 25 خط تولیدی افزوده بررسی شد**؛ Real WP Acceptance `35007660048` روی `f750719` برای هر دو prefix سبز و هرکدام **318/0**؛ Closure `35007660007` سبز. اجرای whole-file WPCS تشخیصی `35006539261` با 814 خطای legacy/PR-wide و 69 هشدار، گیت canonical added-line نبود؛ پس از محدودکردن به delta همین blocker، همهٔ خطوط جدید پاک شدند.
- **کامیت‌های اصلی:** `3d379a6` رفع fail-closed + تست؛ `280eaa8` اصلاح Acceptance؛ `0aed6d5` fixture Organization پویا؛ `f750719` انطباق WPCS خطوط اصلاح. workflow شواهد موقت پس از ثبت خروجی حذف می‌شود.
- **Migration:** جدیدی اضافه نشد؛ آخرین migration همچنان `2026_09_09_0020_idempotency_clinic_scope.php` است و `0021` وجود ندارد.
- **موارد باز/تحویل:** تنها blocker تحویل، ناسازگاری شاخه است: PR #50 شاخهٔ `arena/01a0a5ec-doctor` را دنبال می‌کند ولی این نشست مجاز به push فقط روی `arena/01a0a629-doctor` است؛ PR/branch/Ready/merge دست‌نخورده ماند. پس از cleanup نهایی و push، گیت‌های خودکار exact-final-head ثبت می‌شوند.
- **وضعیت tree:** پس از commit پاک‌سازی نهایی باید clean باشد.
