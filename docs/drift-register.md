...
| **OD-7** | انتقال اجباری `clinic-files` و `cpms-backups` به خارج از DocumentRoot | ⚠️ **باز — جدید در Phase 1A** — روی nginx، `.htaccess` بی‌اثر است. **بازبینی تأییدی: افشای مستقیم در پیکربندی پشتیبانی‌شدهٔ nginx ممکن است ⇒ توصیهٔ گزینهٔ A (همین Phase 1)** |
| **OD-8** | `verify_mobile` از Endpoint عمومی می‌تواند حساب `cpms_patient` بسازد | ⚠️ **باز — جدید در بازبینی تأییدی Phase 1A** — گزینهٔ الف: ساخت حساب فقط با `PURPOSE_LOGIN` (توصیه‌شده) / گزینهٔ ب: حفظ وضع موجود |
