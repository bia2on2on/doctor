...
> Phase 10 was **not started at that checkpoint** and its existing roadmap scope was untouched (documentation-only closure) — **historical; preserved, not rewritten** so that Phase 10 does not appear to have always been started.
>
> **Historical Phase 10 Slice 1 note (was the current note at the `84d6649…` checkpoint; superseded as the current Phase 10 note by the queue-actions closure note below — label changed only, body preserved byte-for-byte; re-verified live 2026-09-24):** Phase 10 — Doctor Portal is **IN PROGRESS — NOT CLOSED**, started by **explicit Owner authorization** (canonical decision record `docs/decisions/2026-09-24-phase10-doctor-portal-owner-authorization.md` — بخش اول). **Phase 10 Slice 1 = CLOSED / TECHNICALLY COMPLETE (BOUNDED)** via **PR #113** (merge `b49a777367a3bdf15f9ad944eab4e65ca70c5b26`; MERGED 2026-09-24T08:27:29Z; parents `ca08c885fee55a6dd83d8755e40b47b9a9277233` (main before the merge = merge of PR #111 — Phase 9 formal closure) + `0ad8b5d5841d75a7992cc53aa22bbe8907fe20cf` = the exact accepted PR #113 head). Bounded scope: independent Doctor Portal shell (no wp-admin/theme chrome) · shared WordPress auth/session foundation · doctor-only portal boundary · server-derived professional identity · trusted Clinic scope · trusted operational Location scope · 0/1/N Location selection behavior (0 eligible ⇒ **fail closed / no data**; 1 eligible ⇒ **auto-resolution allowed**; N>1 ⇒ **explicit Location selection required**; **no first/primary fallback**) · Location is the operational timezone source · Location-local operational day · read-only Today summary · read-only Live Queue · read-only Today's Appointments (**owner-approved bounded read-only addition to the same first slice**) · own-doctor + Clinic + Location isolation · multi-location browser evidence · Persian/RTL/responsive Doctor Portal shell. Post-merge on the exact merge SHA: four required workflows terminal-success — CI `35975354166` · Real WordPress Acceptance `35975354115` · Closure Gate `35975354213` · Pilot/Staging Readiness Gate `35975354133` (+ 19/19 check runs success on the merge SHA and on the accepted head). Migration `2026_09_20_0022_slot_holds_patient_binding.php` **remains** the latest on main (22 files `0001`..`0022`; no `0023`); **PR #113 added no migration**. **Current live main has since advanced to `84d664982517dd98c30678816e068e38c7d541fe`** — the merge of **PR #115**, which is a **test-infrastructure / browser-harness correction only on one non-product file** (a D-class harness fix); **it is not a Phase slice, makes no product scope change and requires no migration churn** (post-merge on that exact merge SHA: CI `35981920078` · Real WordPress Acceptance `35981920109` · Closure Gate `35981920090` · Pilot/Staging Readiness Gate `35981920191` — all terminal-success + 19/19 check runs success). **PR #112 remains historical / superseded — CLOSED / NOT MERGED; PR #113 is the authoritative merged continuation**, preserving the accepted RED ancestry and forward-only fix history. **The stricter N>1 Location requirement is portal-specific** — the established shared / wp-admin queue behavior was preserved and the portal rule must not be misstated as a global rule for every staff queue caller. This note resolves no drift row, promotes no drift row, and creates no new drift row. This is NOT a release, go-live, V1-complete, commercial-completeness, production-deployment or overall-product-completion declaration, and it claims no implementation of: Doctor Portal queue actions (call / start / recall / skip) · broader appointment operations · visit workspace · note authoring UX · prescriptions workflow · handwriting integration · files workflow · patient search · doctor schedule/presence management · doctor practice-location management · finance/visit fee · secretary/cashier workflow · MRN customization/editing. **Phase 10 remains IN PROGRESS**; **next bounded capability: Doctor Portal queue actions — call / start / recall / skip — NOT IMPLEMENTED / pending** (evidence: SRS `FR-18.3` follows `FR-18.2`, and the existing queue state machine/routes already expose these doctor transitions; reusing them lets the secretary/reception side observe doctor call state; **not implemented here**).

> **Current Phase 10 queue-actions (Slice 2) closure note (re-verified live 2026-09-24; current live main `87625a03fd92e966de7b13930fc3fb9651138b9f`):** Phase 10 — Doctor Portal remains **IN PROGRESS — NOT CLOSED**, started by **explicit Owner authorization** (canonical decision record `docs/decisions/2026-09-24-phase10-doctor-portal-owner-authorization.md`). **Phase 10 Slice 2 — Doctor Portal queue actions (Call / Start / Recall / Skip) = CLOSED / TECHNICALLY COMPLETE (BOUNDED)** via **PR #117** (MERGED 2026-09-24T12:56:18Z; merge `87625a03fd92e966de7b13930fc3fb9651138b9f`; merge parents `e9177bcd80c80c59fc08153a6760fb24a3c6c493` (main before the merge = the merge of **PR #116** — Phase 10 Slice 1 bounded documentation closure) + `a73c27d2dbf823754d58ded63e2df2f67af74b84` = the exact accepted PR #117 head; the accepted RED remains historical evidence and is not rewritten). Bounded delivered scope: **Call · Start · Recall · Skip** reusing the **existing queue state machine** — **no new queue state**; **no migration**; the **selected trusted Location is enforced on mutations when Location scope is present**; **cross-Location mutation is rejected non-enumerably** (canonical `404 CLINIC_NOT_FOUND` before any state change, pre-transaction and post-`FOR UPDATE` alike); **legacy/shared no-Location mutation behavior is preserved** (no global Location rule — the PR #112 mistake is not repeated); the existing **transaction / `FOR UPDATE` / state validation** is retained; the **existing recall limit** is retained; the **Skip reason is required**; the **optional existing room text contract** is retained; Doctor Portal action controls are **state-driven**; the **existing nonce / capability / doctor-ownership** checks are retained; secretary/reception observability continues through the **existing queue history / realtime / `QUEUE_CALLED` notification** (**no new realtime architecture**). Post-merge on the exact merge SHA: four required workflows terminal-success — CI `36002282790` · Real WordPress Acceptance `36002282465` · Closure Gate `36002282615` · Pilot/Staging Readiness Gate `36002282432` — all `push`→`main` bound to `87625a0…`, all retrieved terminal-success live, + **19/19 check runs success on the merge SHA and 19/19 on the accepted head `a73c27d…`**; **no queued / in_progress / cancelled run is counted as PASS**. Migration `2026_09_20_0022_slot_holds_patient_binding.php` **remains** the latest on main (22 files `0001`..`0022`; no `0023`); **PR #117 added no migration**. **PR #112 remains historical / superseded — CLOSED / NOT MERGED; PR #113 = Slice 1; PR #115 = test-infrastructure / browser-harness correction only; PR #116 = Slice 1 documentation closure only — none of #115/#116 is a phase slice.** **The stricter N>1 Location requirement is portal-specific** and the portal Location guard must not be misstated as a global rule for every staff queue caller. This note resolves no drift row, promotes no drift row, and creates no new drift row. This is NOT a release, go-live, V1-complete, commercial-completeness, production-deployment or overall-product-completion declaration, and it claims no implementation of: broader appointment operations · visit workspace · note authoring UX · prescriptions workflow · handwriting integration · files workflow · patient search · doctor schedule/presence management · doctor practice-location management · finance / visit fee · secretary/cashier workflow · MRN customization/editing. **Phase 10 remains IN PROGRESS**; **next bounded capability: the Doctor visit workspace — open the selected patient's current visit + established clinical record + private and patient-visible notes — NOT IMPLEMENTED / pending** (Complete/Reopen only if the Owner confirms they are inside the Phase 10 bounded contract; the slice boundary must be fixed by the Owner before starting; live evidence: SRS `FR-18.3` — the «record + current Visit» part after selecting a patient from the queue, its queue-action part now delivered — and `FR-18.4`; doctor wireframe §1→§2; existing backend E7 `record`, E8/E9 notes, E14/E15 complete/reopen and `VisitMachine`; the current portal has no visit/record UI; prescriptions, handwriting, finance and files are **not** silently added). *(The preceding Slice 1 note is historically correct for its own checkpoint and is preserved, not rewritten.)*
...
#### ۸-۱-۱ — bumpِ ratchetِ WPCS (2026-09-20) — بازگرداندنِ معنایِ مستندِ گیت، بدونِ هیچ تضعیفی

- **baselineِ قبلی:** `f23e0c50049f6f3e91f2425005ce8db08067af73`
  (2026-09-09، پذیرشِ Migration Foundation).
- **baselineِ جدید:** `2263ed673a8fdf4ab91e6a7fffae6d82252e4324`
  (main، 2026-09-20 — همان مبنایی که PR #93 و PR #94 از آن ساخته شدند).
- **چرا bump لازم شد (اثبات‌شده، نه فرض):** collectorِ changed-lines
  **از بدوِ تولد صفر خط جمع می‌کرد**: step در `67e1208` (2026-09-09) با
  pathspecِ `"clinic-practice-management/$f"` نوشته شد در حالی که
  `defaults.run.working-directory` از پیش (`5a44e7f`، 2026-09-05) روی همان
  پوشه بود؛ `git diff` مسیر را نسبت به CWD تفسیر می‌کند ⇒ بی‌match، و
  pathspecِ بی‌match برای `git diff` خطا نیست (rc=0، خروجیِ خالی) ⇒ مجموعهٔ
  added-lines همیشه خالی ⇒ فیلترِ added-lines هرگز چیزی را نمی‌سنجید.
  اندازه‌گیریِ زنده روی همان baseline: فرمِ قدیم **۰** کلید، فرمِ اصلاح‌شده
  **۱۵۲۰۹** کلید روی ۹۶ فایلِ in-scope (برابر با `git diff --numstat`).
- **دلتای انباشته در دورهٔ بی‌اثریِ گیت:** `git rev-list --count
  f23e0c5…2263ed6` = **۵۶۵ کامیت** (۴۸۶ غیرِ merge + ۷۹ merge؛ بازهٔ
  2026-09-09 تا 2026-09-20). این عدد در جریانِ کار زنده راستی‌آزمایی شد.
- **آنچه collectorِ اصلاح‌شده افشا کرد:** همان ۱۵۲۰۹ خطِ add‌شدهٔ
  پس‌از‌baseline، دارای **۱۲۴۹۷ نقضِ phpcs** روی خطوطِ add‌شده
  (CI runs `35534433407`/job `106140784287` و `35535392501`/job
  `106143385609`) — یعنی بدهیِ تاریخیِ استانداردها که تا پیش از آن با
  مجموعهٔ خالی ماسک می‌شد.
- **سیاستِ مقرر (Owner-issued):** «کدِ جدید/دست‌خورده نباید نقضِ جدید
  وارد کند» — **ratchet رو‌به‌جلو**؛ بدهیِ legacy نه mass-format می‌شود و
  نه الزاماً باید پاک باشد. بنابراین این bump **یک‌بار بازگرداندنِ همان
  معنایِ مستندِ ratchet** است (نه چشم‌پوشیِ جدید و نه تضعیفِ سیاست).
- **تغییرناپذیریِ تضمین‌ها:** هیچ sniff حذف/تضعیف نشد؛ هیچ مسیرِ محصولی
  exclude نشد؛ هیچ violation‌ای suppress نشد؛ گیت اختیاری نشد؛ رفتارِ
  **fail-closed** collector حفظ شد (baselineِ بی‌resolve / مسیرِ حل‌نشده /
  ناسازگاریِ `--numstat` ⇒ rc=1؛ self-testها ۴/۴ سبز در CI). سیاستِ
  «فایلِ کاملاً جدید = تمام خطوطش» و «فقط خطوطِ add‌شدهٔ in-scope» بدونِ
  تغییر باقی می‌ماند.
- **آینده:** bumpِ baseline همچنان **دستی + مستند** است (همین بند)؛
  ratchetِ فعال از این پس روی دلتای نسبت به `2263ed67…` سنجیده می‌شود.
