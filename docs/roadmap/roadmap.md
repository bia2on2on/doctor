...
| **Phase 2** | **Multi-Clinic Core** | 🚧 **IN PROGRESS** — **C4/C5/C6 ✅ CLOSED** (ادغام در `origin/main` = `099b644`؛ PR #14 MERGED؛ اصلاحیهٔ پس از بستنِ C6 در `248ca10`؛ PR #17 MERGED) · **C7 ✅ CLOSED** (ترمیم S1..S6؛ PR #20 MERGED 2026-09-11؛ merge = `a385d868`؛ **پذیرش/بستن رسمی با تصمیم صریح مالک در 2026-09-11** — فقط دامنهٔ تعریف‌شدهٔ C7) · **C8 ✅ CLOSED به‌عنوان بستهٔ شواهد/مستنداتِ فوندیشن Location** (بدون پیاده‌سازی؛ جغرافیای ایران ⇒ Phase 4؛ مجوزدهی نهایی ⇒ Phase 3) · **C9 🔄 پیاده‌سازیِ محدودشده کامل — READY FOR ARCHITECT MERGE REVIEW** (PR #23 **DRAFT — DO NOT MERGE**، base = `0fd5c27`؛ فقط دو یافتهٔ A/Low که خودِ C7 معرفی کرده بود — `ScheduleService.php:389` ← `04a7a79e`/C7‑S5 و `FinanceService.php:1111` ← `c4cf9024`/C7‑S3 — translation‑ready در مرز REST با حفظ بایت‌به‌بایتِ فارسیِ پیش‌فرض و `code`/status/`data`؛ بدون تغییر Domain/Application، بدون migration، بدون ابزار گارْد جدید؛ **C9 بسته نشده** — بستن رسمی تصمیمِ پس‌از‌ادغام است؛ جزئیات: `phase2-state.md` §C9) · **C10 🕒 NOT STARTED — مجاز نشده** (عملکرد **اندازه‌گیری‌نشده**) · وضعیت جاری: [`project-current-state.md`](../project-current-state.md) |
