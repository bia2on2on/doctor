...
| **Phase 8** | **IN PROGRESS — NOT complete** (bounded post-merge documentation closure at `440ba7e7d07579036128f288f502d390d27c98d3` = current main؛ re-verified live 2026-09-20) — Slice 1 / سطحِ مشاهدهٔ عمومیِ آنونیم (browse) = CLOSED (BOUNDED) — **PR #87**، merge `41225398…` (head `0643e2b5…` exact-head 19/19؛ post-merge روی merge SHA چهار workflow لازم terminal-success)؛ Slice 2 / جریانِ OTP→Hold→Confirm = CLOSED (BOUNDED) — **PR #89**، merge `440ba7e…` (head `70d033cc…` exact-head 19/19؛ post-merge: CI `35502216463` · Real WP `35502216468` · Closure `35502216456` · Pilot/Staging `35502216452` + 19/19)؛ سیاست‌های Hold-Timing و حداقلِ هویتِ بیمارِ جدید **Owner-issued**‌اند (نه عبارتِ اصلی SRS — `docs/decisions/2026-09-20-phase8-slice2-owner-decisions.md`)؛ **`0021` آخرین migration روی main است** (`cpms_otp_tokens.clinic_id`؛ ساخته‌شده با مجوزِ صریحِ مالک برای همین برش)؛ **`0022` روی main وجود ندارد** — فقط رزروِ شاخهٔ PR #90 (draft) است؛ **Slice 3 = فعالِ فقط در draft PR #90** (`c17c965d…`) — نه پذیرفته، نه merge؛ هیچ ادعای بستنِ آن اینجا ثبت نمی‌شود. PR #88 (merge `e00cec8d…`) fixِ hardening زیرساختی است، نه برشِ فاز | بخشی از **V2** → OD-2 باز | drift O-* | V1.5 = PAUSED → OD-2 | **IN PROGRESS** (Slice 1 + Slice 2 CLOSED؛ BOUNDED) |
