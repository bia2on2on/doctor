...
| # | Method/Path | Cap |
|---|---|---|
| G1 | `GET/POST /config/schedules` + `PUT/DELETE /config/schedules/{id}` — برنامه هفتگی هر پزشک (یک رکورد به‌ازای هر (روز هفته، Location)؛ `day_of_week` 0=شنبه..6=جمعه؛ `start_time/end_time` HH:MM؛ `appointment_duration_min` 5–240؛ `slot_capacity` 1–50؛ بازه استراحت اختیاری داخل بازه کاری). **Phase 6 Slice 3:** `POST` (ساخت) `location_id` **الزامی** است — Location باید واقعی، فعال و متعلق به Clinic معتبرِ Scope باشد؛ بیگانه/ناموجود/غیرفعال ⇒ `404 CLINIC_NOT_FOUND` (هم‌پاکت با «محل یافت نشد» — عدم شمارش) و نبودِ فیلد ⇒ `400 CLINIC_VALIDATION_FAILED` `errors.location_id=required` (هیچ جایگزینی بی‌صدا با Location اصلی/اولی انجام نمی‌شود). قاعدهٔ تکراری: یک ردیف به‌ازای هر (Clinic، Location، clinician، `day_of_week`) — همان روز هفته در Location دیگرِ همان Clinic مجاز است (Multi-shift هنوز فعال نیست). `GET`/view مقدار ذخیره‌شدهٔ `location_id` را باز می‌گرداند؛ Slotها Location و timezone محلیِ Location ذخیره‌شده را به‌ ارث می‌برند. تغییر/حذف → حذف Slotهای **خالیِ آینده** و بازتولید اتمیک (ADR-0004)؛ Slot دارای رزرو/hold هرگز حذف نمی‌شود. | `cpms_config` |
