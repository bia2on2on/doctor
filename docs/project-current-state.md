...
This file describes the current integrated main checkpoint `8a324e59` (Phase 9 Slice 3 closure —
Phase 9 = IN PROGRESS, Slices 1–3 = CLOSED (BOUNDED); see the top header block) plus the
immediately preceding `352a0936` (PR #98 — owner-issued Portal Architecture Contract + standalone-shell technical proof; not a phase slice),
...
| Phase 9 | **IN PROGRESS — NOT complete** — bounded post-merge documentation closure at `8a324e59341851f5ced01ed37ecd5e69c725f30a` (= current main; PR #99 MERGED 2026-09-21T14:53:05Z; re-verified live 2026-09-21). **Slice 1 = CLOSED** via **PR #93** (merge `a593958521386d459f76c5efa839d9962e1f67fc`; patient self-cancel from the existing patient-portal surface using the existing B4 backend — no new REST route, no schema change; post-merge on that merge SHA: CI `35539966134` · Real WP `35539966131` · Closure Gate `35539966142` · Pilot/Staging `35539966158`). **Slice 2 = CLOSED / TECHNICALLY COMPLETE (BOUNDED)** via **PR #96** (merge `c18c18f4a33e3690d5002f6b21d56ad60334cdde`; Internal Patient Portal notification list + unread badge + mark-all-read on the existing notification backend — no migration, no new REST route, no provider implementation; post-merge on that merge SHA: CI `35581581186` · Real WP `35581581172` · Closure Gate `35581581149` · Pilot/Staging `35581581181`). **Slice 3 = CLOSED / TECHNICALLY COMPLETE (BOUNDED)** via **PR #99** (merge parents `352a0936…` (merge of **PR #98** — owner-issued Portal Architecture Contract + standalone-shell technical proof; not a phase slice) + `8d6231c19d3caccb94194836040f575a1566d345` (the exact accepted PR #99 head); accepted head `8d6231c1…` exact-head 19/19 check runs success; post-merge on the exact merge SHA: four required workflows terminal-success — CI `35615159555` · Real WordPress Acceptance `35615159562` · Closure Gate `35615159508` · Pilot/Staging Readiness Gate `35615159515` — all `push`→`main` bound to `8a324e59…`, + 19/19 check runs success) — **the independent CPMS Patient Portal shell is now production code on main** (`src/Frontend/PatientPortalShell.php` + plugin-owned standalone template `templates/patient-portal-shell.php` + dedicated portal stylesheet; CPMS-owned WordPress Page + plugin-owned `template_include` + standalone full-document template per the PR #98 contract); **the Patient Portal no longer uses wp-admin as its operational UI** (pure-patient `login_redirect` + wp-admin GET guard → frontend portal) and **the active WordPress Theme does not own the Patient Portal shell presentation**; existing Slice 1/2 content, WordPress session and `wp_rest` nonce reused — **no new REST endpoint, no new authentication system, no migration/schema change**; latest migration remains `2026_09_20_0022_slot_holds_patient_binding.php` (22 files `0001`..`0022`; no `0023`). **Owner visual approval for the Slice-3 shell = COMPLETE at the accepted head** (owner statement recorded by this closure; no separate GitHub review artifact on PR #99). The PR titles retain their pre-merge "test(red) … (RED)" wording (Git history not rewritten); the merged final states carry the GREEN product implementations. **PR #94** (merge `b6c34c11…`) remains a merged CI-infrastructure fix + WPCS ratchet governance, **PR #95** (merge `28f51144…`) / **PR #97** the Slice 1 / Slice 2 documentation-only closures, and **PR #98** the governing architecture contract/proof — none is a phase slice. **Phase 9 as a whole is NOT complete and the Patient Portal is NOT claimed complete; no further Phase 9 slice is scoped, started, or claimed.** This is NOT a release/go-live/V1/commercial claim |

**Integrated main checkpoint:** `8a324e59341851f5ced01ed37ecd5e69c725f30a` (PR #99 MERGED 2026-09-21T14:53:05Z —
Phase 9 Slice 3 closure — Phase 9 = IN PROGRESS, Slice 3 = CLOSED / TECHNICALLY COMPLETE (BOUNDED): the
independent CPMS Patient Portal shell is production code on main (CPMS-owned Page + plugin-owned `template_include`
+ plugin-owned standalone full-document template per the PR #98 contract; Patient Portal no longer operates inside
wp-admin; the active Theme does not own the shell presentation); merge parents `352a0936…` (merge of PR #98 —
owner-issued Portal Architecture Contract + standalone-shell technical proof; not a phase slice) + `8d6231c1…` (the
exact accepted PR #99 head); accepted head `8d6231c1…` exact-head 19/19 check runs success; post-merge on the exact
merge SHA: four required workflows terminal-success — CI `35615159555` · Real WP `35615159562` · Closure Gate
`35615159508` · Pilot/Staging `35615159515` — + 19/19 check runs success; **no new REST endpoint; no new
authentication system; no schema change** — migration `2026_09_20_0022_slot_holds_patient_binding.php` **remains**
the latest on main (22 files `0001`..`0022`; no `0023`); owner visual approval of the Slice-3 shell = COMPLETE at the
accepted head (owner statement); Phase 9 NOT complete, Patient Portal not claimed complete, no further Phase 9 slice
scoped; no release/go-live/V1/commercial claim). The immediately preceding mains: `352a0936…` (PR #98, MERGED
...
Roadmap Phase 9 — Patient Portal = **IN PROGRESS — NOT complete**, **Slices 1–3 = CLOSED (BOUNDED)** via
**PR #93** / **PR #96** / **PR #99** (merge `8a324e59341851f5ced01ed37ecd5e69c725f30a` = current main) — see the top header block.)*
