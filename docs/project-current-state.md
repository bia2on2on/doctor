...
>
> **Integrated main checkpoint (Phase 10 — Doctor Portal = IN PROGRESS; Phase 10 Slice 1 = CLOSED / TECHNICALLY COMPLETE (BOUNDED) via PR #113; Phase 10 Slice 2 / queue actions = CLOSED / TECHNICALLY COMPLETE (BOUNDED) via PR #117; re-verified live 2026-09-24):** `87625a03fd92e966de7b13930fc3fb9651138b9f` — "Merge pull request #117 from bia2on2on/arena/01a0d314-doctor" (MERGED 2026-09-24T12:56:18Z; merge parents `e9177bcd80c80c59fc08153a6760fb24a3c6c493` (main before the merge = the merge of **PR #116** — Phase 10 Slice 1 bounded documentation closure) + `a73c27d2dbf823754d58ded63e2df2f67af74b84` — the exact accepted PR #117 head). Post-merge on the exact merge SHA `87625a0…`: four required workflows terminal-success — CI `36002282790` · Real WordPress Acceptance `36002282465` · Closure Gate `36002282615` · Pilot/Staging Readiness Gate `36002282432` — all `push`→`main` runs bound to `87625a0…`, all retrieved terminal-success live; **19/19 check runs `completed`/`success` on the merge SHA** and 19/19 on the accepted head `a73c27d…`; **no queued / in_progress / cancelled run is counted as PASS**. Migration `2026_09_20_0022_slot_holds_patient_binding.php` remains the latest (22 files `0001`..`0022`; no `0023`).
...
| Phase 10 | **IN PROGRESS — NOT CLOSED.** **Slice 2 / queue actions = CLOSED / TECHNICALLY COMPLETE (BOUNDED)** via **PR #117** (MERGED 2026-09-24T12:56:18Z; merge `87625a03fd92e966de7b13930fc3fb9651138b9f`; accepted head `a73c27d2dbf823754d58ded63e2df2f67af74b84`; post-merge on the exact merge SHA: four required workflows terminal-success — CI `36002282790` · Real WP `36002282465` · Closure Gate `36002282615` · Pilot/Staging `36002282432` — + 19/19 check runs success on the merge SHA and 19/19 on the accepted head; **no migration** — latest migration remains `2026_09_20_0022_slot_holds_patient_binding.php`; 22 files `0001`..`0022`; no `0023`). Bounded delivered scope: **Call · Start · Recall · Skip** reusing the **existing queue state machine** (**no new queue state**); selected trusted Location enforced on mutations when Location scope is present; cross-Location mutation rejected **non-enumerably**; **legacy/shared no-Location mutation behavior preserved**; existing transaction / `FOR UPDATE` / state validation retained; existing recall limit retained; Skip reason required; optional existing room text contract retained; Doctor Portal action controls are **state-driven**; existing nonce / capability / doctor-ownership retained; secretary/reception observability continues through the existing queue history / realtime / `QUEUE_CALLED` notification (**no new realtime architecture**); **no migration**. The Owner **explicitly authorized Phase 10 start** (decision record: [`decisions/2026-09-24-phase10-doctor-portal-owner-authorization.md`](decisions/2026-09-24-phase10-doctor-portal-owner-authorization.md) — بخش اول). **Slice 1 = CLOSED / TECHNICALLY COMPLETE (BOUNDED)** via **PR #113** (MERGED 2026-09-24T08:27:29Z; merge `b49a777367a3bdf15f9ad944eab4e65ca70c5b26`; accepted head `0ad8b5d5841d75a7992cc53aa22bbe8907fe20cf`; post-merge on the exact merge SHA: four required workflows terminal-success — CI `35975354166` · Real WP `35975354115` · Closure Gate `35975354213` · Pilot/Staging `35975354133` — + 19/19 check runs success; **no migration** — latest migration remains `2026_09_20_0022_slot_holds_patient_binding.php`; 22 files `0001`..`0022`; no `0023`). Bounded delivered scope: **independent Doctor Portal shell** (no wp-admin/theme chrome) · **shared WordPress auth/session foundation** · **doctor-only boundary** · **server-derived professional identity** · **trusted Clinic scope** · **trusted operational Location scope** · **0/1/N Location behavior** (0 ⇒ fail closed / no data; 1 ⇒ auto-resolution; N>1 ⇒ **explicit Location selection required**; **no first/primary fallback**) · **Location = operational timezone source** · **Location-local operational day** · **read-only Today summary** · **read-only Live Queue** · **read-only Today's Appointments** · **own-doctor + Clinic + Location isolation** · **Persian/RTL/responsive shell**. **The stricter N>1 Location requirement is portal-specific** (`VisitService::todayForDoctorPortal()`); the established shared/wp-admin queue behavior was preserved (`VisitService::today()`, still serving `/secretary/today` and `/queue`) and must not be misstated as a global rule. **PR #112 = CLOSED / NOT MERGED (historical / superseded); PR #113 = authoritative merged product continuation.** **PR #115 = test-infrastructure / browser-harness correction only — no product scope change; NOT a Phase slice and not a slice of any phase** (it produced the historical checkpoint `84d6649…`; **PR #116 = documentation closure only**). **Doctor Portal queue actions (call / start / recall / skip) left this pending list when PR #117 closed the bounded Slice 2 slice.** **Explicitly NOT claimed implemented (pending):** broader appointment operations · visit workspace · note authoring UX · prescriptions workflow · handwriting integration · files workflow · patient search · doctor schedule/presence management · doctor practice-location management · finance / visit fee · secretary/cashier workflow · MRN customization/editing. **Next bounded capability: the Doctor visit workspace — open the selected patient's current visit + the established clinical record + private and patient-visible notes — NOT IMPLEMENTED / pending** (Complete/Reopen only if the Owner confirms they are inside the Phase 10 bounded contract). **The Owner must fix the slice boundary before implementation** (prescriptions, handwriting, finance and files exist in the broader doctor workflow and are **not** silently added to this slice). Live evidence: SRS `FR-18.3` — the «record + current Visit» part after selecting a patient from the queue (its queue-action part is now delivered) — and `FR-18.4` immediately after it; doctor wireframe §1→§2; existing backend E7 `record`, E8/E9 notes, E14/E15 complete/reopen and `VisitMachine` (`complete`/`reopen`); the current portal has no visit/record UI. The Owner-issued broader product intent (stable intent only, no speculative role/permission contract): the product must serve both a small practice (doctor + secretary, one/few Locations) and a larger medical organization (multiple Clinics/Locations/doctors/staff). This is NOT a release/go-live/V1/commercial claim |
