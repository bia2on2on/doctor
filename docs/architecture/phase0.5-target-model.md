...
> **پیاده‌سازی C5 (2026-09-09):** `normalize()` ارقام یونیکد را هم حالت می‌کند —
> فارسی `۰۱۲۳۴۵۶۷۸۹` و عربی `٠١٢٣٤٥٦٧٨٩` پیش از هر چیز به ASCII تا می‌شوند
> (`fold_digits`). فرم‌های «98 + صفرِ میان‌شهری» (`+98 (0912) 123-4567` =
> ۱۳رقم `9809...` و ۱۵رقم `009809...`) هم پذیرفته می‌شوند. یک اصلاح رفتار:
> شاخهٔ قدیمیِ ۱۲رقمیِ `9809...` خروجی خراب `00...` می‌ساخت (هرگز شکل استاندارد
> نبود) — حالا آن ورودی `null` است و شکل معتبر ۱۳رقمی پذیرفته می‌شود
> (تست‌های table-driven در `tests/Unit/MobileValidatorTest.php`). دامنه همچنان
> فقط ایران است؛ تصمیم E.164 بین‌المللی باز مانده (بالا).
