...
### 1.1 هدف
طراحی و پیاده‌سازی یک افزونه اختصاصی WordPress برای مدیریت کلینیک/مطب پزشک شامل: احراز هویت بیمار (Mobile+OTP)، رزرو نوبت آنلاین با کنترل Race Condition، پرونده الکترونیک بیمار، مدیریت مراجعه و صف مطب، ویزیت و یادداشت پزشکی (به‌همراه دست‌خط قلم و تشخیص متن)، نسخه، توصیه و Follow-Up، امور مالی (Invoice/Payment)، اعلان، Audit و گزارش‌گیری — با معماری Production-Ready.

**تصمیم محصول نهایی (ADR-0027):** **یک محصول واحد** برای هر دو سناریو «مطب تک‌پزشکی» و «درمانگاه چندپزشکی» — یک Core، یک Schema، Features/UX تطبیقی (حالت مطب = زیرمجموعه ساده‌شده حالت درمانگاه؛ انتخاب پزشک با یک Clinician فعال خودکار Skip می‌شود). دو Plugin/Codebase/Fork جداگانه ممنوع. بازبینی آمادگی: `docs/architecture/multi-doctor-readiness-review.md` (بدون تغییر Foundational؛ Backlog هم‌ترازی به فازها نگاشت شده).

### 1.2 محدوده
- **داخل محدوده (سیستم):** تمام فرایند Section 51 Master Prompt (سناریوی پذیرش اصلی).
- **داخل محدوده معماری (از روز اول — ADR-0003/0027):** چندپزشکی/چندمنشی در تمام مدل داده (`clinic_id` + `clinician_id`)؛ قابلیت‌های چندپزشکی کامل (Specialty/Department/Branch/Staff-Scope/گزارش Breakdown) طبق roadmap در فاز خودشان فعال می‌شوند.
- **خارج محدوده V1:** پرداخت آنلاین، بیمه، اتصال آزمایشگاهی، نسخه الکترونیک ملی، اپ موبیو Native، شعبه فعال (معماری آماده است، فعال‌سازی در V2).
...
| **Doctor** | پزشک؛ Dashboard Tablet/Pen-First (نه Tablet-Only — Responsive) | PHI **زمینه خودش**: ویزیت/Notes خصوصی خودش (Ownership سرور-side)؛ Private Notes پزشک دیگر هرگز (ADR-0027). دید Clinic-wide فقط با Scope صریح (V2 — ADR-0026) |
...
|---|---|---|
| A-1 | سررسید V1: یک شعبه و عموماً یک پزشک فعال — ولی محصول **یک محصول واحد چندپزشکی** است (ADR-0027): حالت مطب = زیرمجموعه UX همان معماری؛ معماری چند-پزشک/شعبه از روز اول فعال است. | نیاز به Migration کم‌خطر (ستون clinic_id از قبل وجود دارد). |
...
| A-5 | ~~در V1 پزشک می‌تواند به Private Notes سایر پزشکان هم دسترسی داشته باشد (تیم واحد).~~ **ابطال‌شده توسط پیاده‌سازی F5 + تأیید ADR-0027:** مالکیت «ویزیت خودش» در سرویس‌های بالینی اعمال می‌شود؛ «پزشک بودن در همان درمانگاه» به‌طور خودکار به Private Notes/ویزیت پزشک دیگر دسترسی نمی‌دهد. | — (رفتار سخت‌گیرانه‌تر از فرض اولیه؛ تست‌شده از F5) |
...
| FR-3.10 | **الزام صریح مالک (مدت نوبت/اسلات قابل‌پیکربندی):** Scheduling باید «مدت نوبت/اسلات» را برای برنامهٔ کاری پزشک **قابل پیکربندی** کند — نمونه‌های لازم: **۱۵ دقیقه**، **۲۰ دقیقه** و سایر مقادیر معتبرِ تعریف‌شده توسط اپراتور. این الزام **قصد ندارد** یک مقدار سراسریِ واحد را بر همهٔ پزشکان/کلینیک‌ها/شعبه‌ها تحمیل کند. **وضعیت as-built (بازرسی کد، 2026-09-17 — بدون ادعای تکمیل تجاری/UX):** مدل موجود همین رابطه را امن بیان می‌کند و **نیازی به معماری «مدت» جدید نیست**: ستون هر ردیفِ برنامهٔ هفتگی `cpms_schedule.appointment_duration_min` (`SMALLINT UNSIGNED NOT NULL DEFAULT 20` — Migration `0001`)، اعتبارسنجی **۵..۲۴۰ دقیقه** (`ScheduleService`)، مسیر اپراتور در wp-admin (فیلد مدت در فرم برنامهٔ کاری) و REST (`POST/PUT /config/schedules`)، مصرف در تولید Slot و در `DurationResolver` طبق ADR-0017 (Override نوبت ← Service ← برنامهٔ پزشک ← پیش‌فرض کلینیک + Snapshot در زمان رزرو). کار باقی‌مانده صرفاً **UX/پیش‌فرض‌های تجاریِ آنبوردینگ** است (فاز مالک: Phase 20). | M |
...
| FR-4.11 (ADR-0027) | رزرو چندپزشکی: جریان‌های پشتیبانی‌شده = «پزشک → خدمت → زمان» (فعلی) و در فاز مربوطه «تخصص → خدمت → پزشک → زمان» و «اولین پزشک آزاد» — همه روی همان مدل Slot per-clinician (بدون Redesign). انتخاب پزشک وقتی فقط یک Clinician فعال است خودکار Skip می‌شود (حالت مطب). | C (V1.5/V2 طبق roadmap) |
...
### 3.23 راه‌اندازی اولیهٔ تجاری (Setup Wizard / Onboarding)

| ID | نیازمندی | اولویت |
|---|---|---|
| FR-23.1 | **الزام صریح مالک:** افزونهٔ تجاری باید پس از نصب/فعال‌سازی، یک **ویزارد راه‌اندازی هدایت‌شده (Setup Wizard)** برای اپراتور فراهم کند تا محصول برای اولین استفادهٔ واقعی به‌درستی پیکربندی شود. این یک الزامِ «راه‌اندازی هدایت‌شدهٔ اولین اجرا» است، **نه صرفاً مستندات**. مراحل، صفحه‌ها، پیش‌فرض‌ها و UX دقیق ویزارد **هنوز تأیید نشده‌اند و نباید اختراع شوند**؛ پیاده‌سازی آینده باید معماری `Organization → Clinic → Location` را رعایت کند (ADR-0031) و **هیچ هویت tenant جعلی/پیش‌فرض نسازد** (AD-13). | M |
| FR-23.2 | **چک‌پوینت الزامی پیش از پیاده‌سازی تجاری:** پیش از پیاده‌سازی نهایی/تجاری ویزارد راه‌اندازی، گرفتن **جهت‌گیری محصولی/UX مالک** دربارهٔ مراحل دقیق ویزارد الزامی است: «Before final commercial Setup Wizard implementation, obtain owner product/UX direction for exact wizard steps.» وجودِ ویزارد فنیِ موجود به‌معنای کامل‌شدن تجاریِ این الزام نیست؛ هیچ ادعای تکمیل بدون شواهد مجاز نیست. | M |

> **وضعیت as-built (بازرسی کد، 2026-09-17 — فقط شاهد وجود فنی، نه تکمیل تجاری):** یک ویزارد فنیِ ۱۲گامی وجود دارد: `CpmsSetupWizard` در `clinic-practice-management/src/Admin/CpmsSetupWizard.php` (slug `cpms-wizard`، capability `cpms_config`، resumable از طریق `setup.current_step`، گیت تکمیل = نام کلینیک + ≥۱ پزشک فعال؛ مستند در [`admin-ux-plan.md`](../admin-ux-plan.md) §Chunk B و با تست `SetupWizardTest`). این ویزارد **Organization/Clinic/Location جدید نمی‌سازد**؛ فقط Clinic موجودِ scope را می‌خواند/به‌روزرسانی می‌کند. بنابراین «ویزارد فنی موجود» ≠ «تکمیل الزام تجاری آنبوردینگ».
> **فاز مالکِ نهایی‌سازی تجاری:** **Phase 20 — UI/UX & Commercial Release** (طبق [`roadmap.md`](../roadmap/roadmap.md) §۰). این ثبت به‌خودی‌خود شروع کار، تأیید UX، یا تغییر در کار فعالِ فاز جاری نیست.
