...
| بالینی | `NOTE_CREATE`, `NOTE_UPDATE` (version), `RX_CREATE`, `RX_FINALIZE`, `RX_VOID`, `REC_CREATE`, `FOLLOWUP_CREATE` |
| دست‌خط/OCR | `HW_DOC_CREATE`, `HW_PAGE_SAVE`, `OCR_JOB`, `OCR_CONFIRMED`, `OCR_REJECTED` |
| فایل | `FILE_UPLOAD`, `FILE_READ` (حساس: private), `FILE_DELETE` |
| مالی | `INVOICE_CREATE`, `INVOICE_VOID`, `PAYMENT_CAPTURE`, `PAYMENT_VOID`, `PAYMENT_REFUND`, `PAYMENT_ADJUST` |
| دسترسی/سیستم | `PERMISSION_GRANT`, `PERMISSION_REVOKE`, `FORBIDDEN_ACCESS_ATTEMPT`, `EXPORT` (filters), `SETTING_UPDATE`, `SCHEDULE_UPDATE` |
